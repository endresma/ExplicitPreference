...
public class OrderedGraph {
    ...
    private void init() {
        ...
        if (!severalOrders) {
            ...
            for (Object obj : ((HashMap<Object, ArrayList<Integer>>) orderSignatures.get(currentOrder)).keySet()) {
                ArrayList<Object> list = new ArrayList<>();
                list.add(obj);
                signatureAssignments.put(list, ((HashMap<Object, ArrayList<Integer>>) orderSignatures.get(currentOrder)).get(obj));
            }
    ...
    private void buildStandardGraph() {
        ...
        for (int i = 0; i < orderedpairList.size(); i++) {
            ...
            if (orderedpairList.get(i).getChild() != null && !(graph.containsVertex(orderedpairList.get(i).getChild()))) {
            ...
            if (orderedpairList.get(i).getParent() != null && !(graph.containsVertex(orderedpairList.get(i).getParent()))) {

                graph.addVertex(orderedpairList.get(i).getParent());

            }
            if (orderedpairList.get(i).getParent() != null && !parents.contains(orderedpairList.get(i).getParent())) {
            ...
            if (orderedpairList.get(i).getParent() != null && orderedpairList.get(i).getChild() != null) {
                ...
                System.out.println(orderedpairList.get(i).getParent() + " -> " + orderedpairList.get(i).getChild());
            }
        }
        DirectedGraph<Object, DefaultEdge> temp_gr = new DefaultDirectedGraph<Object, DefaultEdge>(DefaultEdge.class);
    ...
    private void depthSearch() {
        ...
        GraphIterator<Object, DefaultEdge> iterator_depthFirst = new DepthFirstIteratorEdge<>(graph, VIRTUAL_TOPNODE, 0);

        while (iterator_depthFirst.hasNext()) {
            iterator_depthFirst.next();
        }

        depthResult = ((DepthFirstIteratorEdge<Object, DefaultEdge>) iterator_depthFirst).getDepthResults();
    ...
    private void breadthSearch() {
        ...
        GraphIterator<Object, DefaultEdge> iterator_breathFirst = new BreadthFirstIterator_<>(graph, VIRTUAL_TOPNODE, 4, length, depthResult, SIGNATURE_METHOD);
    ...
    private HashMap<Object, ArrayList<Integer>> removePosition(int i, HashMap<Object, ArrayList<Integer>> resultTuple) {
    ...
    protected void combineSignatures() {
        ...
        for (int i = 0; i < orders.length; i++) {
            ...
            Set<Object> set = ((HashMap<Object, ArrayList<Integer>>) orderSignatures.get(order)).keySet();
            Iterator<Object> iter = set.iterator();
            while (iter.hasNext()) {
                Object key = iter.next();
                list.add(key);
                signaturesOfSomeOrder.put(key, ((HashMap<Object, ArrayList<Integer>>) orderSignatures.get(order)).get(key));
        ...
        for (int j = 0; j < combinedKeys.size(); j++) {
            ...
            for (int k = 0; k < combinedKeys.get(j).length; k++) {
                ...
                einzeln_signatur = list_of_signatures.get(k).get(combinedKeys.get(j)[k]);
                for (int z = 0; z < einzeln_signatur.size(); z++) {
                    signature_kombiniert[len] = einzeln_signatur.get(z).intValue();
                    len++;
                }
            }
    ...
    private void createCombinations(ArrayList<ArrayList<Object>> keys, int index, Object[] values) {
